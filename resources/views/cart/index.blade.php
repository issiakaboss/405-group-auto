<x-guest-layout>
    <div x-data="{ 
        ...
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('public/cart.title') }}</h1>

            <!-- Bouton Tout Nettoyer -->
            <button type="button"
                id="btn-clear-cart"
                x-data
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative item-row" data-id="{{ $id }}">
                            ...
                            <div class="space-y-3 w-full sm:w-auto">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-400 font-medium mt-0.5">
                                        {{ $item['year'] ?? '' }} &bull; {{ number_format((float)($item['mileage'] ?? 0)) }} {{ __('public/cart.miles') }}
                                    </p>
                                    <p class="text-xs text-gray-400 mt-0.5">
                                        {{ $item['category'] ?? '' }} &bull; {{ $item['fuel_type'] ?? __('public/cart.default_fuel') }}
                                    </p>
                                </div>

                                <div class="flex flex-wrap items-center gap-2 pt-1">
                                    <a href="{{ route('vehicles.show', $item['id'] ?? $id) }}" class="text-xs font-semibold px-3.5 py-2 border border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 transition shadow-sm">{{ __('public/cart.view_details') }}</a>
                                    ...
                                    <button
                                        ...
                                        {{ __('public/cart.schedule_test_drive') }}
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex sm:flex-col items-center sm:items-end justify-between sm:justify-between w-full sm:w-auto sm:h-28 border-t sm:border-t-0 pt-4 sm:pt-0 border-gray-100">
                            <button type="button"
                                ...
                                title="{{ __('public/cart.remove_item') }}">
                ...
                <div class="space-y-6">
                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-5 sticky top-6">
                        <h3 class="font-bold text-gray-900 text-base">{{ __('public/cart.selection_summary') }}</h3>

                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between text-gray-500">
                                <span id="summary-text">{{ __('public/cart.vehicles_selected') }} (<span id="summary-count">{{ count($cart) }}</span> <span id="summary-unit">{{ count($cart) > 1 ? __('public/cart.cars_plural') : __('public/cart.car_singular') }}</span>)</span>
                                <span class="font-semibold text-gray-900" id="summary-subtotal">${{ number_format($total) }}</span>
                            </div>
                            <div class="flex justify-between text-gray-500">
                                <span>{{ __('public/cart.inquiry_review') }}</span>
                                <span class="text-emerald-600 font-bold">{{ __('public/cart.no_payment') }}</span>
                            </div>

                            <hr class="border-gray-100 my-4">

                            <div class="flex justify-between text-lg font-black text-gray-900">
                                <span>{{ __('public/cart.total') }}</span>
                        ...
                        <div class="space-y-2.5 pt-2">
                            <a href="{{ route('checkout.index') }}" class="w-full py-3.5 px-4 bg-[#0F172A] hover:bg-gray-800 text-white font-semibold text-xs rounded-xl shadow-sm transition tracking-wide flex items-center justify-center space-x-2 text-center">
                                <span>{{ __('public/cart.submit_selection') }}</span>
                            </a>
                            <a href="{{ route('home') }}" class="w-full inline-block text-center py-3 text-xs font-semibold text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition shadow-sm">{{ __('public/cart.continue_shopping') }}</a>
                        </div>
                        <p class="text-[11px] text-gray-400 text-center">{{ __('public/cart.no_payment_notice') }}</p>
                    </div>
                </div>
        ...
        <div id="empty-cart-state" class="{{ count($cart) === 0 ? '' : 'hidden' }} text-center py-20 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
            ...
            <p class="text-sm text-gray-500 font-medium mb-4">{{ __('public/cart.empty_cart') }}</p>
            <a href="{{ route('home') }}" class="inline-flex text-xs font-semibold px-5 py-2.5 bg-[#0F172A] text-white rounded-xl hover:bg-gray-800 transition shadow-sm">{{ __('public/cart.continue_shopping') }}</a>
        </div>
        ...
        <div x-show="testDriveModal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" x-cloak x-transition>
            <div @click.away="testDriveModal = false" class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4">
                <div class="flex justify-between items-center">
                    <h3 class="font-bold text-gray-900 text-base">{{ __('public/cart.modal.test_drive_title') }}</h3>
                    <button @click="testDriveModal = false" class="text-gray-400 hover:text-gray-600 font-bold text-xl">&times;</button>
                </div>
                <p class="text-xs text-gray-500">{{ __('public/cart.modal.test_drive_desc') }} <span class="font-bold text-gray-900" x-text="selectedVehicleTitle"></span>.</p>
                ...
                <form action="{{ route('testdrive.store') }}" method="POST" class="space-y-4 text-xs">
                    ...
                    <div>
                        <label class="block font-semibold text-gray-700 mb-1">{{ __('public/cart.modal.preferred_date') }}</label>
                    ...
                    <div>
                        <label class="block font-semibold text-gray-700 mb-1">{{ __('public/cart.modal.preferred_time') }}</label>
                    ...
                    <div>
                        <label class="block font-semibold text-gray-700 mb-1">{{ __('public/cart.modal.special_requirements') }}</label>
                        <textarea name="notes" placeholder="{{ __('public/cart.modal.requirements_placeholder') }}" class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 bg-gray-50 focus:bg-white focus:border-gray-400 transition h-20 resize-none outline-none"></textarea>
                    </div>
                    ...
                    <button type="submit" class="w-full py-3 bg-[#0F172A] hover:bg-gray-800 text-white font-semibold rounded-xl transition mt-2 shadow tracking-wide">
                        {{ __('public/cart.modal.confirm_appointment') }}
                    </button>
                </form>
            </div>
        </div>
        ...
        <x-confirm-modal
            ...
            :title="__('public/cart.modal.delete_item_title')"
            :message="__('public/cart.modal.delete_item_message')"
            :confirmText="__('public/cart.modal.delete_item_confirm')"
            type="danger" />

        <x-confirm-modal
            name="clear-cart"
            :title="__('public/cart.modal.clear_cart_title')"
            :message="__('public/cart.modal.clear_cart_message')"
            :confirmText="__('public/cart.modal.clear_cart_confirm')"
            type="danger" />
    </div>

    <!-- TRANSMISSION DES TRADUCTIONS DE BLADE VERS JS -->
    <script id="cart-translations" type="application/json">
        {
            "carsPlural": "{{ __('public/cart.cars_plural') }}",
            "carSingular": "{{ __('public/cart.car_singular') }}"
        }
    </script>

    <!-- SCRIPT AJAX PANIER -->
    <script>
        // Extraction propre des langues sans mélanger les syntaxes Blade/JS
        const translations = JSON.parse(document.getElementById('cart-translations').textContent);
        const carsLabelPlural = translations.carsPlural;
        const carLabelSingular = translations.carSingular;
        ...
        function updateUI(data) {
            ...
            if (data.cart_count !== undefined) {
                ...
                if (unitElem) unitElem.textContent = data.cart_count > 1 ? carsLabelPlural : carLabelSingular;
        ...
        // Écoute suppression d'un item (Compatible Alpine.js $dispatch)
        window.addEventListener('confirmed-delete-cart-item', function(event) {
            const itemId = Array.isArray(event.detail) ? event.detail[0] : event.detail;
