...
<body class="font-sans text-gray-900 antialiased bg-white" x-data="{ searchResults: [], searchFocused: false }">
    ...
    <nav x-data="{ mobileMenuOpen: false }" class="border-b border-slate-200 bg-slate-100/95 backdrop-blur-md sticky top-0 z-50 transition-all shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="flex items-center text-gray-700">
                    ...
                    <!-- MOBILE MENU -->
                    <button type="button" @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden p-2 rounded-lg transition hover:text-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500" :aria-expanded="mobileMenuOpen.toString()" aria-controls="mobile-menu" aria-label="{{ __('public/guest.mobile_menu') }}">
            ...
            <div id="mobile-menu" x-show="mobileMenuOpen" x-cloak @click.away="mobileMenuOpen = false" class="md:hidden border-t border-slate-200 py-3">
                <div class="space-y-1 text-sm font-semibold">
                    <a href="{{ route('home') }}" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 {{ request()->routeIs('home') ? 'bg-amber-50 text-amber-600' : 'text-gray-700 hover:bg-white hover:text-amber-500' }}">{{ __('public/guest.nav_home') }}</a>
                    <a href="{{ route('home') }}#catalog" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.footer_catalog') }}</a>
                    <a href="{{ route('home') }}#testimonials" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.nav_testimonials') }}</a>
                    <a href="{{ route('about') }}" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 {{ request()->routeIs('about') ? 'bg-amber-50 text-amber-600' : 'text-gray-700 hover:bg-white hover:text-amber-500' }}">{{ __('public/guest.nav_about') }}</a>
                    <a href="{{ route('home') }}#vehicle-request" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.service_finder') }}</a>
                    <div class="grid grid-cols-2 gap-2 border-t border-slate-200 pt-3 mt-2">
                        <a href="{{ route('favorites.index') }}" @click="mobileMenuOpen = false" class="rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.mobile_favorites') }}</a>
                        <a href="{{ route('cart.index') }}" @click="mobileMenuOpen = false" class="rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.footer_cart') }}</a>
                    </div>
                    <div class="grid grid-cols-2 gap-2 border-t border-slate-200 pt-3 mt-2">
                        <a href="{{ route('lang.switch', 'en') }}" @click="mobileMenuOpen = false" class="rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500 {{ app()->getLocale() === 'en' ? 'bg-amber-50 text-amber-600 font-bold' : '' }}">English</a>
                        <a href="{{ route('lang.switch', 'fr') }}" @click="mobileMenuOpen = false" class="rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500 {{ app()->getLocale() === 'fr' ? 'bg-amber-50 text-amber-600 font-bold' : '' }}">Français</a>
                    </div>
                    @auth
                    <div class="border-t border-slate-200 pt-3 mt-2 space-y-1">
                        <p class="px-3 py-2 text-xs font-bold uppercase tracking-wider text-gray-500">{{ Auth::user()->name }}</p>
                        @role('admin')
                        <a href="{{ route('admin.vehicles.index') }}" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.admin_fleet') }}</a>
                        <a href="{{ route('admin.orders.index') }}" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.admin_orders') }}</a>
                        @endrole
                        @role('user')
                        <a href="{{ url('/dashboard') }}" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.user_dashboard') }}</a>
                        @endrole
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full text-left rounded-lg px-3 py-2 text-red-600 hover:bg-red-50">{{ __('public/guest.logout') }}</button>
                        </form>
                    </div>
                    @endauth
                    @guest
                    <a href="{{ route('login') }}" @click="mobileMenuOpen = false" class="block rounded-lg px-3 py-2 text-gray-700 hover:bg-white hover:text-amber-500">{{ __('public/guest.mobile_login') }}</a>
                    @endguest
                </div>
            </div>
        </div>
                    ...
                    <ul class="space-y-3 text-xs">
                        ...
                        <li>
                            <a href="mailto:405autogroup@gmail.com" class="flex items-center gap-2.5 hover:text-white transition-colors">
                                ...
                                <span class="truncate">info@405autogroup.com</span>
                            </a>
                        </li>
                    </ul>
                </div>
