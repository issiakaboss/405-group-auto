        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <h2 class="font-extrabold text-2xl text-white">
                    {{ __('admin/orders.order_details', ['id' => str_pad($order->id, 5, '0', STR_PAD_LEFT)]) }}
                </h2>
                <a href="{{ route('admin.orders.index') }}" class="text-xs font-semibold px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition">
                    &larr; {{ __('admin/orders.back_to_list') }}
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2 space-y-6">
                    <div class="bg-slate-800/50 border border-slate-700/50 p-6 rounded-2xl shadow-xl backdrop-blur-xl">
                        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{{ __('admin/orders.sections.ordered_items') }}</h3>
                        <div class="divide-y divide-slate-700/50">
                            ...
                            <div class="py-3 flex justify-between items-center text-xs">
                                <div>
                                    ...
                                    <p class="text-slate-400">{{ __('admin/orders.fields.quantity', ['qty' => $item->quantity]) }}</p>
                                </div>
                        ...
                        <div class="mt-6 pt-4 border-t border-slate-700/50 flex justify-between items-center">
                            <span class="text-xs font-bold text-slate-400 uppercase">{{ __('admin/orders.fields.total_amount') }}</span>
                ...
                <div class="space-y-6">
                    <div class="bg-slate-800/50 border border-slate-700/50 p-6 rounded-2xl shadow-xl backdrop-blur-xl">
                        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{{ __('admin/orders.sections.customer_info') }}</h3>
                        <div class="space-y-3 text-xs">
                            <div>
                                <span class="text-slate-400 block">{{ __('admin/orders.fields.name') }}</span>
                                <span class="font-semibold text-white">{{ $order->customer_name }}</span>
                            </div>
                            <div>
                                <span class="text-slate-400 block">{{ __('admin/orders.fields.email') }}</span>
                                <span class="font-semibold text-white">{{ $order->customer_email }}</span>
                            </div>
                            <div>
                                <span class="text-slate-400 block">{{ __('admin/orders.fields.phone') }}</span>
                                <span class="font-semibold text-white">{{ $order->phone }}</span>
                            </div>
                            <div>
                                <span class="text-slate-400 block">{{ __('admin/orders.fields.address') }}</span>
                    ...
                    <div class="bg-slate-800/50 border border-slate-700/50 p-6 rounded-2xl shadow-xl backdrop-blur-xl">
                        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{{ __('admin/orders.sections.update_status') }}</h3>
                        <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST" class="space-y-4">
                            @csrf
                            @method('PATCH')
                            @php $currentStatus = $order->status->value ?? $order->status; @endphp
                            ...
                            <button type="submit" class="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs rounded-xl shadow-lg transition">
                                {{ __('admin/orders.buttons.update_order') }}
                            </button>
                        </form>
                    </div>
                </div>
