        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <a href="{{ route('admin.vehicles.index') }}" class="text-xs font-bold text-slate-400 hover:text-white transition flex items-center space-x-1 uppercase tracking-wider">
                    ← {{ __('admin/vehicles.back_to_fleet') }}
                </a>
            </div>

            <div class="bg-[#0f172a] border border-slate-800/80 shadow-2xl rounded-2xl p-8 md:p-10">
                <div class="mb-8">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-indigo-400 block">{{ __('admin/vehicles.sourcing_stock') }}</span>
                ...
                <form action="{{ route('admin.vehicles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 text-xs font-semibold text-slate-300">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.year') }}</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.vehicle_type') }}</label>
                            <select name="vehicle_type" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs" required>
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_type') }}</option>
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.body_style') }}</label>
                            <select name="body_style" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs" required>
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_style') }}</option>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.exterior_color') }}</label>
                            <select name="exterior_color" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs" required>
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_exterior') }}</option>
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.interior_color') }}</label>
                            <select name="interior_color" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs" required>
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_interior') }}</option>
                                @foreach(\App\Models\Enums\VehicleColor::cases() as $color)
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.transmission') }}</label>
                            <select name="transmission" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs">
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_transmission') }}</option>
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.fuel_type') }}</label>
                            <select name="fuel_type" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs">
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_fuel') }}</option>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.price_usd') }}</label>
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.mileage_miles') }}</label>
                        ...
                        <div>
                            <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.location') }}</label>
                            <select name="location" class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white transition text-xs" required>
                                <option value="" class="bg-[#1e293b]">{{ __('admin/vehicles.select_location') }}</option>
                    ...
                    <!-- 6. Description -->
                    <div>
                        <label class="block text-slate-400 mb-1.5 uppercase tracking-wider text-[11px]">{{ __('admin/vehicles.description') }}</label>
                        <textarea name="description" rows="3" placeholder="..." class="w-full p-3 bg-[#1e293b] border border-slate-700/60 rounded-xl focus:border-indigo-500 focus:ring-0 text-white placeholder-slate-500 transition text-xs">{{ old('description') }}</textarea>
                        @error('description') <p class="text-red-400 text-[10px] mt-1">{{ $message }}</p> @enderror
                    </div>

                    <!-- 7. Téléversement de photos -->
                    <div class="pt-4 border-t border-slate-800">
                        <label class="block text-slate-200 text-sm font-bold mb-2">{{ __('admin/vehicles.add_photos') }}</label>
